...
void renderText(SDL_Renderer *renderer, const char *text, int y, SDL_Color color, int fontSize) {
    TTF_Font *font = TTF_OpenFont("src/font/ByteBounce.ttf", fontSize);
...
void renderButton(SDL_Renderer *renderer, int y) {
    ...
    SDL_Rect button = {(800 - width) / 2, y, width, height};
    drawFilledCircle(renderer, 314, y + 32, 33);
    drawFilledCircle(renderer, 480, y + 32, 33);
...
void createPauseButton(SDL_Renderer *renderer) {
    ...
    SDL_Rect line1 = {655, 90, 6, 20};
    SDL_RenderFillRect(renderer, &line1);
    SDL_RenderDrawRect(renderer, &line1);

    SDL_Rect line2 = {665, 90, 6, 20};
